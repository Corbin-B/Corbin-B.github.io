...
function bindPlugins(){
    $(document).on('changeState',function(e,d){        
        var index = 0;
        if (!isIE9) {
            index = $('.onepage-pagination>li>a[href="'+location.pathname+'"]').parent().index();
        } else {
            if (location.hash === ''){ 
                index = 0; 
            } else {
                index = $('.onepage-pagination>li>a[href="'+location.pathname+String(location.hash).substr(2,location.hash.length)+'"]').parent().index();
            }
        }
        switch(d){
            case '':
                index = 0;
                $(document).trigger('changeLocation','home');
            break;
        }        
        $("#other_pages").moveTo(index+1);
        $('.onepage-pagination>li>a')
            .removeClass('active')
            .eq(index).addClass('active');
        $('#menuNav>ul>li').removeClass('sfHover')
    });
}
